...
public class ClientActor extends Actor {
    ...
    public void run() {
	while (true) {
	    Port port = new Port(serverName, "192.168.100.101");
		...
		System.out.println(getActorName()
			+ " - invio messaggio: request(ACQUIRE, " + port + ")");
	    ...
	    System.out
		    .println(getActorName() + " - risorsa " + r + " allocata");
		...
		System.out.println(getActorName()
			+ " - invio messaggio: request(RELEASE, " + port + ", "
			+ r + ")");
	    } catch (UnknownHostException e) {
		e.printStackTrace();
	    } catch (IOException e) {
