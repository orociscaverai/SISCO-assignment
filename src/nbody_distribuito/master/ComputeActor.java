			...
			try {
				...
				while(waitCompleteJobs != 0){
					...
					if(res.getType().equalsIgnoreCase("stop")){
						...
						return; 
						//La Mailbox sarà clearata in seguito or TODO switch mailbox per sicurezza
					}else if (res.getType().equalsIgnoreCase("ResultCompute")){
						AbstractNode n = tr.navigateInOrder();
						if(n!=null){
							//TODO per il send serve identificare l'attore che ha finito la computazione
							//quindi aggiungere un id al messaggio ResultCompute
							int id = 0;
							BodiesMap map = (BodiesMap) n.getValue();
							send(workers.elementAt(id), new Message("DoJob",map));
						}else{
							waitCompleteJobs--;
						}
				...
				//TODO tutti i job sono completati sendare newMap a qualche attore?
